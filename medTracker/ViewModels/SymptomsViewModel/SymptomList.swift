    ...
    class SymptomList : ObservableObject {
        ...
        // Only the favorite symptoms, sorted by their creation date
        var sortedFavoriteSymptoms: [Symptom] {
            sortedSymptoms.filter { $0.isFavorite }
        }

        // Only the non-favorite symptoms, sorted by their creation date
        var sortedNonFavoriteSymptoms: [Symptom] {
            sortedSymptoms.filter { !$0.isFavorite }
        }
